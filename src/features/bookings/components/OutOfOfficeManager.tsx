...
export function OutOfOfficeManager() {
  ...
  const handleDelete = async (id: string) => {
    if (confirm("Are you sure you want to delete this absence period?")) {
              ...
              {editingId === block.id ? (
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="font-medium text-sm">{format(new Date(block.start_at), "dd MMM yyyy", { locale: it })} – {format(new Date(block.end_at), "dd MMM yyyy", { locale: it })}</div>
                    {block.reason && <p className="text-sm text-muted-foreground mt-1 truncate">{block.reason}</p>}
                  </div>
                  <div className="flex items-center gap-1">
                    <Button variant="ghost" size="sm" onClick={() => startEdit(block)}><Pencil className="h-4 w-4" /></Button>
                    <Button variant="ghost" size="sm" onClick={() => handleDelete(block.id)} disabled={deleteMutation.isPending}><Trash2 className="h-4 w-4" /></Button>
                  </div>
                </div>
              )}
            </div>
