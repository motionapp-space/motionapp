...
export interface GroupedClientNotifications {
  today: ClientNotification[];
  yesterday: ClientNotification[];
  lastWeek: ClientNotification[];
  older: ClientNotification[];
}

export function groupClientNotificationsByDate(
  notifications: ClientNotification[]
): GroupedClientNotifications {
  const now = new Date();
  const sevenDaysAgo = startOfDay(subDays(now, 7));
  const twoDaysAgo = startOfDay(subDays(now, 2));

  const result: GroupedClientNotifications = {
    today: [],
    yesterday: [],
    lastWeek: [],
    older: [],
...
export function getGroupLabel(key: keyof GroupedClientNotifications): string {
  switch (key) {
    case "today":
      return "Oggi";
    case "yesterday":
