...
import { useClientNotificationsQuery } from "../hooks/useClientNotificationsQuery";
...
export function ClientNotificationBell() {
  ...
  const { data: notifications = [] } = useClientNotificationsQuery();
  const { markOne, markAll } = useClientMarkAsRead();
  ...
  const handleViewAll = () => {
    setOpen(false);
    navigate("/client/app/notifications");
  };
